...
SDK.prototype.getCurrentPublicKey = function(accessToken) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        return reject(new Error("Don't find a valid public key or the public key associated with the current access token no longer exists or has expired."));
    }.bind(this));
};
 ...
 *
 * @param  {Object} tokenContent Token payload
 * @param  {String} cert         Private key for sign the token
 * @param  {String} [lifetime]   Optional lifetime of the token
 * @return {String}              Token generated
 */
SDK.prototype.signToken = function(tokenContent, cert, lifetime) {
    if (lifetime) {
        return (jwt.sign(tokenContent, cert, {algorithm: "RS256", expiresIn: lifetime}));
    }
...
SDK.prototype.verifyToken = function(accessToken) {
    ...
    return this.getCurrentPublicKey(accessToken).then(function (certificateCurrent) {
        ...
        if (null === payload) {
            return Promise.reject(new Error("Access token's payload is null"));
        }
        return payload;
    });
};
