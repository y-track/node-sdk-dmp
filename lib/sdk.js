...
SDK.prototype.verifyScopes = function (accessToken, scopes) {

	var matches = this.decodeScopes(accessToken).match(/[a-z]+:[a-z0-9*]*:[a-zA-Z0-9*]*/g);
	if (null !== matches) {
		if (matches.indexOf(scopes) >= 0) {
			return true;
		} else {
			return false;
		}
